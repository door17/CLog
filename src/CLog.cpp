...
public:
    ...
    static int transform(int level)
    {
        ...
        if ((level < 0) || (level >= sizeof(s_levels)))
        {
...
struct CLogContext
{
    ...
    void setLogger(CLog::LoggerType type, CLog::Writer* logger, int flags)
    {
...
CLog::Level CLog::setLevel(Level level)
{
    detail::AutoLock lock(s_context.cs);

    std::swap(s_context.level, level);
    return level;
}
...
int CLog::setLogger(LoggerType type, int flags)
{
    int rc = 0;
    ...
    if (type == COUT)
    {
        s_context.setLogger(type, new detail::LogWriter(detail::writeCout), flags);
    }
    else if (type == SYSLOG)
    {
#ifndef WIN32
        s_context.setLogger(type, new detail::SyslogWriter(), flags);
#endif //WIN32
    }
    else if (type == DEBUGWINDOW)
    {
        s_context.setLogger(type, new detail::LogWriter(detail::writeDebug), flags);
    }
    else if (type == FILE)
    {
        s_context.setLogger(type, new detail::FileLogWriter(s_context.fileName), flags);
    }
    else
    {
        rc = EINVAL;
...
void CLog::setExtLogger(WriteLog logger, int flags)
{
    detail::AutoLock lock(s_context.cs);

    if (s_context.logger[EXTERNAL])
    {
        delete s_context.logger[EXTERNAL];
        s_context.logger[EXTERNAL] = NULL;
    }

    s_context.logger[EXTERNAL] = new detail::LogWriter(logger);
    s_context.flags[EXTERNAL] = flags;
}
...
int CLog::reload(const char* configPath)
{
    ...
    std::string strLevel;
    iniFile.queryValue("CLog", "Level", strLevel);
    ...
    if (enabled)
    {
        setLogger(CLog::DEBUGWINDOW, flags);
    }
...
void CLog::setExtLogger(Writer* logger, int flags)
{
    detail::AutoLock lock(s_context.cs);
    ...
    s_context.logger[EXTERNAL] = logger;
    s_context.flags[EXTERNAL] = flags;
}
