...
public:
    ...
    /**
     * 设置日志输出级别, 仅当要输出的日志级别 > 设置值 时，日志才真正输出
     * @param level 日志级别
     * @return 原先的日志级别
     */
    ...
    /**
     * 获取当前配置的日志级别， 默认值为 kNone，即输出所有日志
     * @return
     */
     ...
     * @return 错误码， 0 表示成功
     */
    static int setLogger(LoggerType type, int flags = DATETIME);
     ...
     * @param logger
     */
    static void setExtLogger(WriteLog logger, int flags);
     ...
     */
    static void setExtLogger(Writer* logger, int flags);
